...
SELECT  mentorship_eligibility.title, COUNT(*)
INTO qualified_retirement_ready_employees
FROM mentorship_eligibility
GROUP BY mentorship_eligibility.title
ORDER BY COUNT(*) DESC;

SELECT * FROM qualified_retirement_ready_employees;
...
ON employees.emp_no = titles.emp_no
WHERE (dept_emp.to_date = '9999-01-01')
AND (birth_date BETWEEN '1964-01-01' AND '1967-12-31')
ORDER BY emp_no,to_date DESC;
SELECT * FROM mentorship_eligibility2
DROP TABLE mentorship_eligibility2 CASCADE;
SELECT DISTINCT ON (emp_no) emp_no,
	first_name,
	last_name,
	birth_date,
...
SELECT  qualified_mentors.title, COUNT(*)
INTO qualified_mentors_count
FROM qualified_mentors
GROUP BY qualified_mentors.title
ORDER BY COUNT(*) DESC;

SELECT * FROM qualified_mentors_count;
